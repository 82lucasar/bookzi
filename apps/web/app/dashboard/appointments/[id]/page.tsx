      ...
      <div style={{ padding: "20px 16px 140px", maxWidth: 560, margin: "0 auto", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ background: "white", borderRadius: 20, overflow: "hidden", border: "1.5px solid #E0F0F8", boxShadow: "0 1px 6px rgba(0,0,0,0.05)" }}>
          <div style={{ display: "flex", alignItems: "stretch" }}>
            <div style={{ background: "linear-gradient(180deg, #0284C7, #0369A1)", padding: "24px 18px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minWidth: 76 }}>
              <span style={{ fontSize: 34, fontWeight: 900, color: "white", lineHeight: 1 }}>{day}</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.75)", textTransform: "capitalize", marginTop: 4 }}>{month.slice(0, 3)}</span>
              <span style={{ fontSize: 11, color: "rgba(255,255,255,0.55)", marginTop: 2 }}>{year}</span>
            </div>
            <div style={{ flex: 1, padding: "20px 18px", minWidth: 0 }}>
              <p style={{ fontSize: 20, fontWeight: 800, color: "#0F172A", letterSpacing: "-0.3px", marginBottom: 4 }}>{appt.clientName}</p>
              <p style={{ fontSize: 14, color: "#64748B", fontWeight: 500, marginBottom: 6 }}>{appt.serviceName}</p>
              <p style={{ fontSize: 18, fontWeight: 700, color: "#0284C7", marginBottom: 12 }}>{startTime} — {endTime}</p>
              <span style={{ display: "inline-flex", alignItems: "center", padding: "6px 14px", borderRadius: 20, background: sc.bg, color: sc.text, border: `1.5px solid ${sc.border}`, fontSize: 13, fontWeight: 700 }}>
                {sc.label}
              </span>
            </div>
          </div>
        ...
        <div style={{ background: "white", borderRadius: 20, border: "1.5px solid #E0F0F8", padding: "20px", boxShadow: "0 1px 6px rgba(0,0,0,0.05)" }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#64748B", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 14 }}>Cliente</p>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
            <div style={{ width: 48, height: 48, borderRadius: 14, background: "linear-gradient(135deg, #0284C7, #0369A1)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 800, fontSize: 16, flexShrink: 0 }}>
              {initials}
            </div>
            <div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {waHref && (
              <a href={waHref} target="_blank" rel="noreferrer" style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px 0", borderRadius: 12, background: "rgba(37,211,102,0.08)", color: "#128C7E", fontWeight: 700, fontSize: 13, textDecoration: "none", border: "1.5px solid rgba(37,211,102,0.2)" }}>
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1C3.69 1 1 3.69 1 7c0 1.04.27 2.02.74 2.87L1 13l3.23-.73A5.97 5.97 0 007 13c3.31 0 6-2.69 6-6S10.31 1 7 1z" stroke="currentColor" strokeWidth="1.3"/></svg>
                WhatsApp
              </a>
            )}
            {emailHref && (
              <a href={emailHref} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px 0", borderRadius: 12, background: "rgba(2,132,199,0.08)", color: "#0284C7", fontWeight: 700, fontSize: 13, textDecoration: "none", border: "1.5px solid rgba(2,132,199,0.2)" }}>
        ...
        {appt.priceSnapshot && (
          <div style={{ background: "white", borderRadius: 20, border: "1.5px solid #E0F0F8", padding: "20px", boxShadow: "0 1px 6px rgba(0,0,0,0.05)" }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: "#64748B", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 10 }}>Precio</p>
        ...
        {appt.paymentProofUrl && (
          <PaymentProofSection
            ...
            startTime={startTime}
            startDateLabel={`${day} ${month.slice(0, 3)}`}
          />
        )}
